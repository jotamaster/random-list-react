...
//esta arrow function es de ant design 
    ...
    _handleKeyPress = (e) => {
        if (e.key === 'Enter') {
         this.props.addOp(document.getElementById('registro'));
    ...
    render() {
        ...
        return (
                ...
                <Card title="Agrega un registro a la lista" style={{ width: '100%', marginTop: 20, backgroundColor: '' }}>
                    <Row gutter={16}>
                        <Col sm={24} md={14}>
                            <Input onKeyPress={this._handleKeyPress} id="registro" style={{marginBottom:10}} placeholder="Nuevo Registro" />
                        </Col>
                        <Col sm={24} md={6}>
                            {/* el onclick dispara el evento que se encuentra en el contexto de este componenete */}
                            <Button onClick={() => { this.props.addOp(document.getElementById('registro')); document.getElementById('registro').value = null} }   type="primary" icon="plus" size={size}>Agregar</Button>
                        </Col>
                    </Row>
                </Card>
